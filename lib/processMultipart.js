...
const tryCleanAllParsedFiles = require('./tryCleanAllParsedFiles');
const {
  isFunc,
  debugLog,
  buildFields,
...
module.exports = (options, req, res, next) => {
  ...
  const propagateError = (err) => {
    tryCleanAllParsedFiles(options, req).then(() => {
      if (isFunc(options.errorHandler)) {
  ...
  busboy.on('file', (field, file, info) => {
    ...
    const sendErrorEventToFile = (err) => {
      ...
      isFunc(file.destroy) ? file.destroy(err) : file.emit('error', err);
    };
    ...
    file.on('limit', () => {
      ...
      uploadTimer.clear();
      // stop parsing request and send error event to do cleanup.
      req.unpipe(busboy);
      req.resume();
      sendErrorEventToFile(
        new FileUploadLimitError(
  ...
  busboy.on('finish', () => {
    ...
    // Fork Change#2
    // convert file object to array of file object if not already
    if (req.files) {
      const filesKeys = Object.keys(req.files);
      for (let i = 0; i < filesKeys.length; i++) {
        let key = filesKeys[i];
        if (req.files[key] && !(req.files[key] instanceof Array)) {
  ...
  busboy.on('error', (err) => {
    debugLog(options, `Busboy error: ${String(err)}`);
    propagateError(err);
  });
